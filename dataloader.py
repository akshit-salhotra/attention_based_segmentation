...
class HumanDataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        #     img = img[:,:,np.newaxis]
        #     mask  = mask[:,:,np.newaxis]
        sample = {"imidx":imidx, "image": img, "mask":mask, "rawimg":raw_image}
...
class HumanDataset_akshit(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        #     img = img[:,:,np.newaxis]
        #     mask  = mask[:,:,np.newaxis]
        sample = {"imidx":imidx, "image": img, "rawimg":raw_image}
        if self.transforms is not None:
            sample = self.transforms(sample)
        return sample
...
if __name__ == '__main__':
    ...
    crack_dataset = HumanDataset(
        img_path = paths['img_path'],
        mask_path = paths['mask_path'],
		transforms = transforms.Compose([Rescale(300), RandomFlip(), ToTensor(flag = 1)]) #transforms.Compose([Rescale(300), RandomFlip(), ToTensor(flag = 0)
        )
    print(crack_dataset[0]['mask'].shape, np.unique(crack_dataset[0]['mask']))
